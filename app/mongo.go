...
*/
func tikMongo(client mongo.Client, cn string, newUser *CreateUser) bool {
	collection := client.Database("monGo").Collection(cn)
	filter := bson.D{{"id", newUser.Id}, {"username", newUser.UserName}}
	check, err := collection.Find(context.TODO(), filter)
	if err != nil {
		log.Fatal(err)
	}

	if newUser.PW != newUser.PWConfirm || check.Next(context.TODO()) {
		fmt.Println("Create Failed!")
		return false
	}
	user := &User{Id: newUser.Id, Password: newUser.PW, UserName: newUser.UserName, CreatedAt: time.Now()}
